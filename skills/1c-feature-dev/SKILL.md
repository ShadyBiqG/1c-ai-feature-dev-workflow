...
- Этот workflow работает через штатные средства Конфигуратора и инструменты, настроенные проектом.
- EDT MCP не является предусловием работы: не проверяй его доступность и не останавливай задачу из-за его отсутствия.
- Метаданные, синтаксис и результат доработки проверяй средствами Конфигуратора. Если нужная проверка недоступна, явно зафиксируй ограничение в evidence, но не требуй EDT MCP.
...
|---|---|---|---|---|
| Controller / Manager | основной агент | `gpt-6-sol` | `high` | Оценка сложности, маршрутизация, контроль фаз и итог |
| Discovery | `1c-explorer` | `gpt-6-luna` | `high` | Метаданные, существующий код и зависимости |
| Architecture | `1c-architect` | `gpt-6-sol` | `high` | Архитектурные решения и инварианты |
| Plan | `1c-planner` | `gpt-6-sol` | `high` | Декомпозиция, критерии приёмки и test-plan |
| Plan Review | `1c-arch-reviewer` | `gpt-6-astra` | `high` | Независимое ревью архитектуры и плана |
| Work: BSL | `1c-developer` | `gpt-6-luna` | `high` | Изменение исходников BSL |
| Work: metadata | `1c-metadata-manager` | `gpt-6-luna` | `high` | Изменение метаданных и форм |
| Fix | `1c-error-fixer` | `gpt-6-luna` | `high` | Исправление ошибок валидации |
| Refactoring | `1c-refactoring` | `gpt-6-luna` | `high` | Локальное упрощение без изменения поведения |
| Test Generation | `1c-code-test-generator` | `gpt-6-luna` | `high` | Создание тестов по подробному test-plan через `$1c-testing` |
| Test Execution | `1c-tester` | `gpt-6-luna` | `high` | Запуск YAxUnit / Vanessa и фиксация evidence |
| Code Review | `1c-code-reviewer` | `gpt-6-luna` | `high` | Чеклист и локальные изменения; см. правила выбора модели |
| Documentation | `1c-doc-writer` | `gpt-6-luna` | `high` | Пользовательская и техническая документация |
| Deploy | `1c-tester` | `gpt-6-luna` | `high` | Применение в тестовой ИБ и smoke-тест после подтверждения |

### Выбор модели для тестов и ревью

- По умолчанию генерируй тесты и проводи локальное чеклист-ревью на `gpt-6-luna high`, когда требования, test-plan и затронутые модули определены.
- Используй `gpt-6-sol high` для генерации тестов, если сначала нужно восстановить поведение, определить границы или учесть интеграции; для ревью — если требуется анализ регрессий, зависимостей, транзакций либо соответствия архитектуре.
- Используй `gpt-6-astra high` только для сложного архитектурного ревью и критичных рисков.
...
Если Work на `gpt-6-luna high` дважды не проходит один и тот же gate, повтори исправление на `gpt-6-sol high`. Если причина архитектурная, верни задачу в Plan на `gpt-6-sol high`.
...
**Лимит**: не более трёх проходов ревью, включая первый.
...
2. Если агент находит проблемы на первом или втором проходе:
   - Обнови план в `.tasks/task-[feature-name]/phase4-architecture.md`
   - **Повтори Phase 5** (ревью обновлённого плана)
3. Если после третьего прохода остаются проблемы, сохрани их в `phase5-plan-review.md` и представь пользователю: нерешённые пункты, их последствия и риски. Не запускай четвёртый проход и не переходи к Phase 6 без решения пользователя.
4. Когда план одобрен агентом, сохрани результаты ревью в `.tasks/task-[feature-name]/phase5-plan-review.md`:
...
2. Запусти `1c-code-test-generator` на `gpt-6-luna high`, если test-plan подробный. Если требуется восстановить поведение, определить границы либо учесть интеграции, используй `gpt-6-sol high`. Передай требования, архитектуру, test-plan, выполненные этапы и список изменённых файлов.
3. Генератор меняет только тесты и тестовые данные. Продуктивный код ему менять запрещено.
4. Запусти отдельного `1c-tester` на `gpt-6-luna high` для выполнения YAxUnit / Vanessa Automation и сохранения evidence.
...
5. **Если "исправить сейчас"**:
   ...
   - **Вернись к Phase 7** для повторного запуска затронутых тестов, затем повтори ревью; всего допускается не более трёх проходов ревью.
6. Если после третьего прохода остаются проблемы, представь пользователю нерешённые пункты, последствия и риски. Не запускай четвёртый проход автоматически; дождись решения пользователя о дальнейших действиях.
7. **Если "исправить позже"**:
...
2. После подтверждения запусти `1c-tester` на `gpt-6-luna high` и используй установленный проектом штатный способ обновления базы.
